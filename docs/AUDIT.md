...
|---|---|---|---|---|
| MED | **Required** — layout corruption if `sizeof` differs | `atomic_ref` reinterprets `T` as `std::atomic<T>` with no size guard; if an implementation made `std::atomic<T>` larger than `T`, an atomic op would R/W past the object. (An initial `static_assert(is_always_lock_free)` was too strict — it broke Cortex-M0, where `std::atomic<uint32_t>` is size-compatible but not lock-free, and std::atomic_ref itself works on non-lock-free `T` via an external lock pool.) | `atomic_ref.hpp:41` | ✅ `static_assert(sizeof(std::atomic<T>) == sizeof(T))`; alignment still enforced at construction; `is_always_lock_free` exposed for callers |
